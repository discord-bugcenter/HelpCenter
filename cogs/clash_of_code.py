...
class COCDiscord(COC):
    def __init__(self, data: dict, message: discord.Message = None, author: discord.Member = None):
    ...
    @classmethod
    def from_coc(cls, coc: COC, message: discord.Message = None, author: discord.Member = None):
...
class COC(commands.Cog):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if match := COC_URL.match(message.content):
            try: coc_message = await self.process_coc(self.get_coc(match.group(1)), message.author)
    ...
    def create_embed(coc_discord: COCDiscord) -> discord.Embed:
        ...
        if coc_discord.author:
            embed.set_author(
                ...
                icon_url=coc_discord.author.avatar.url
            )
    ...
    async def get_coc(self, code) -> COC:
        ...
        if not valid: raise COCLinkNotValid("https://www.codingame.com/clashofcode/clash/" + code)

        return coc

    async def process_coc(self, coc: COC, author: discord.User) -> discord.Message:
